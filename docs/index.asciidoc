...
| <<plugins-{type}s-{plugin}-archive_bucket_id>> |<<string,string>>|No
...
[id="plugins-{type}s-{plugin}-archive_bucket_id"]
===== `archive_bucket_id`

  * Value type is <<string,string>>
  * Default is: `nil`

Should the log file be moved to `archive_bucket_id` after its contents have been updated?
The log file location inside remains the same between buckets.
Metadata is kept.
